...
# ------------------------------------------------------------
# 1. '어제' 날짜를 한국 시간(KST) 기준으로 계산하기
#    - 배포 서버의 시계는 한국 시간이 아닐 수 있으므로
#      반드시 ZoneInfo("Asia/Seoul")로 현재 시각을 구한 뒤 하루를 뺀다.
# ------------------------------------------------------------
now_kst = datetime.now(ZoneInfo("Asia/Seoul"))
yesterday_kst = now_kst - timedelta(days=1)
target_dt = yesterday_kst.strftime("%Y%m%d")  # KOBIS가 요구하는 yyyymmdd 형식

st.caption(f"조회 기준일(한국시간): {yesterday_kst.strftime('%Y년 %m월 %d일')}")
...
def fetch_box_office(target_dt: str, api_key: str):
    ...
    # 3-6. 영화 목록이 비어 있는 경우 (예: 너무 이른 날짜, 집계 전 등)
    if not movie_list:
        return False, (
            "❌ 해당 날짜의 박스오피스 데이터가 없습니다.\n\n"
            "**확인해야 할 것**\n"
            "- 조회 날짜가 너무 최근이라 아직 집계되지 않았을 수 있습니다.\n"
            "- KOBIS 시스템 점검 중일 수 있으니 잠시 후 다시 시도하세요."
        )
